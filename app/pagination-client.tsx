...
export default function PaginationClient() {
  useEffect(() => {
    ...
    const setup = () => {
      ...
      // Find the actual lead grid from the first article after the heading.
      // This is more robust than relying on a particular wrapper/sibling structure.
      const articles = Array.from(document.querySelectorAll('article')) as HTMLElement[];
      const firstArticle = articles.find((article) => Boolean(heading.compareDocumentPosition(article) & Node.DOCUMENT_POSITION_FOLLOWING));
      const grid = firstArticle?.parentElement as HTMLElement | null;
      if (!grid) return;
      ...
      cards.forEach((card, index) => {
        ...
        if (visible) addContactTools(card);
      });
      ...
      const makeButton = (label: string, page: number, disabled = false) => {
        ...
        button.onclick = () => {
          currentPage = page;
          setup();
          window.scrollTo({ top: heading.getBoundingClientRect().top + window.scrollY - 20, behavior: 'smooth' });
        };
    ...
    const schedule = () => { if (timer) clearTimeout(timer); timer = setTimeout(setup, 50); };
    setup();
