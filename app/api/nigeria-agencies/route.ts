...
function parseDirectory(text:string,url:string,source:string,fallbackCity='',fallbackState='Nigeria'){
 ...
 for(const raw of blocks){
  ...
  if(!block||/url source:|title: list of travel agencies/i.test(block))continue;
  ...
  if(!name||name.length<3||name.length>120||BAD.test(name))continue;
...
export async function GET(request:Request){
 const {searchParams}=new URL(request.url); const limit=4000;
 ...
 const groups=await batched([...jobs,...businessJobs,...branchesJobs],8,([u,s,c,st])=>fetchReader(u,s,c,st));
 const osm=await osmPromise; const osmRecords=(osm.elements||[]).map((e:any)=>{const t=e.tags||{};const name=clean(t.name||t['name:en']);const lat=e.lat??e.center?.lat,lon=e.lon??e.center?.lon,c=city(t),st=state(t);return{id:`osm-${e.type}-${e.id}`,name,city:c,state:st,address:clean([t['addr:housenumber'],t['addr:street'],t['addr:suburb'],c,st].filter(Boolean).join(', '))||undefined,phone:clean(t.phone||t['contact:phone']||t['contact:mobile'])||undefined,email:clean(t.email||t['contact:email'])||undefined,website:clean(t.website||t['contact:website'])||undefined,latitude:lat,longitude:lon,services:['Travel agency'],source:'OpenStreetMap · Overpass API',verification:'OPENSTREETMAP LISTED',sourceUrl:`https://www.openstreetmap.org/${e.type}/${e.id}`}}).filter((x:any)=>x.name&&!/helipad|airport operations|bus transport/i.test(x.name));
 const seen=new Set<string>(); const agencies=[...osmRecords,...groups.flat()].filter((x:any)=>{x.name=clean(x.name).replace(/^(?:reviews|photos|sponsored|verified|\|)+/i,'').trim();if(!x.name||x.name.length<3||BAD.test(x.name)||/^(ik[o]?rodu|iwofe|osolo way|reviews|photos)$/i.test(x.name))return false;const key=`${x.name}|${x.city}|${x.state}|${x.phone}`.toLowerCase().replace(/[^a-z0-9|]+/g,'');if(seen.has(key))return false;seen.add(key);return true}).slice(0,limit).map((x:any,i:number)=>({...x,id:x.id||`dir-${i}-${clean(x.name).toLowerCase().replace(/[^a-z0-9]+/g,'-')}`}));
 return NextResponse.json({source:SOURCE,sourceCountClaim:agencies.length,fetchedPages:jobs.length+businessJobs.length+branchesJobs.length+1,count:agencies.length,agencies});
}
