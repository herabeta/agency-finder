...
export async function GET(request:Request){
 ...
 const groups=await batched([...jobs,...abujaDistrictJobs,...businessJobs,...branchesJobs,...goAfricaJobs],8,([u,s,c,st])=>fetchReader(u,s,c,st));
 ...
 return NextResponse.json({source:SOURCE,sourceCountClaim:agencies.length,fetchedPages:jobs.length+abujaDistrictJobs.length+businessJobs.length+branchesJobs.length+goAfricaJobs.length+1,count:agencies.length,agencies});
}
