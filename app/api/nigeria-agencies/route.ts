...
export async function GET(request:Request){
 const {searchParams}=new URL(request.url); const limit=Math.min(4000,Math.max(100,Number(searchParams.get('limit')||1600)));
 ...
 const groups=await batched([...jobs,...businessJobs],8,([u,s,c,st])=>fetchReader(u,s,c,st));
