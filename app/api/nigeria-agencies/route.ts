...
function parseDirectory(text:string,url:string,source:string,fallbackCity='Abuja',fallbackState='Federal Capital Territory'){
 ...
 for(const raw of blocks){
  ...
  let cut=namePart.length; for(const p of cuts){const m=namePart.search(p);if(m>2)cut=Math.min(cut,m)}
  const name=normalizeName(namePart.slice(0,Math.min(cut,120))).replace(/^(?:review|more info|write a review|photos|sponsored|verified)\s*/i,'').trim();
  ...
  const tail=phonePos>name.length?block.slice(name.length,phonePos):''; const address=clean(tail).replace(/\b(?:Image|Verified|Sponsored|Reviews|Photos)\b/gi,'').slice(0,350);
