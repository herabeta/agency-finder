...
function invalidName(name:string){const n=normalizeName(name);return !n||n.length<3||n.length>80||BAD.test(n)||NON_AGENCY.test(n)||DIRECTORY_NAV.test(n)||SENTENCE.test(n)||/^!?(?:image|img)\s*\d*$/i.test(n)||DESCRIPTION.test(n)||/^(?:gsm|tel|fax|phone|mobile)\s*[:(]/i.test(n)||/^(?:home|directory|tourism|travel|contact|profile)\b/i.test(n)||/^(?:previous|next|more|enlarge|digital|centralized|instant|customer|online|professional|showcase|discover)\b/i.test(n)||/^\+?[0-9().\s-]+$/.test(n);}
