...
const ABUJA_DISTRICTS: Array<[string,string]> = [
  ['wuse','Wuse'],['gwarinpa','Gwarinpa'],['maitama','Maitama'],['jabi','Jabi'],['asokoro','Asokoro']
];
...
export async function GET(request:Request){
 ...
 const abujaDistrictJobs=ABUJA_DISTRICTS.map(([slug,c])=>[`https://www.finelib.com/cities/abuja/abuja-cadastral-and-districts/${slug}/travel-agencies`,'Finelib Nigeria · Abuja district Travel Agencies',c,'Federal Capital Territory'] as const);
 const abujaPageJobs=Array.from({length:10},(_,i)=>{const p=i+1;return [`https://www.finelib.com/cities/abuja/travel/travel-agencies/page-${p}`,'Finelib Nigeria · Abuja Travel Agencies', 'Abuja','Federal Capital Territory'] as const});
 const abujaBusinessJobs=Array.from({length:17},(_,i)=>{const p=i+1;return [p===1?'https://www.businesslist.com.ng/category/travel-agents/city%3Aabuja':`https://www.businesslist.com.ng/category/travel-agents/${p}/city%3Aabuja`,'BusinessList Nigeria · Abuja Travel Agents','Abuja','Federal Capital Territory'] as const});
 ...
 const groups=await batched([...jobs,...abujaDistrictJobs,...abujaPageJobs,...abujaBusinessJobs,...businessJobs,...branchesJobs,...goAfricaJobs],8,([u,s,c,st])=>fetchReader(u,s,c,st));
 ...
 return NextResponse.json({source:SOURCE,sourceCountClaim:agencies.length,fetchedPages:jobs.length+abujaDistrictJobs.length+abujaPageJobs.length+abujaBusinessJobs.length+businessJobs.length+branchesJobs.length+goAfricaJobs.length+1,count:agencies.length,agencies});
}
